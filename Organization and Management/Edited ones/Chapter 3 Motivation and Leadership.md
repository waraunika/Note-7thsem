# MOTIVATION
## Introduction to Motivation
**Etymology:** Comes from Latin word _'Mover'_ which means "to move."

**Definition:**  
The process of influencing or stimulating a person to take action by creating a work atmosphere wherein the goals of the organization and needs of the person are satisfied.

**Key Concepts:**
- Internal and external factors that stimulate desire and energy in people
- Makes people continually interested in and committed to their job
- Different people have different forms of motivation (rewards, punishment, monetary incentives)
---
## Role of Management in Motivation

| Role                           | Description                                                                                                |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------- |
| **Understanding Subordinates** | Managers must understand what motivates their subordinates to be more productive                           |
| **Creating Environment**       | Create workspace where individuals can work efficiently with work satisfaction                             |
| **Studying Individual Needs**  | Every human being is unique; managers should study nature and best motivation strategy for each individual |
| **Providing Training**         | Provide necessary training, knowledge, and information about jobs                                          |
| **Gathering Feedback**         | Get necessary feedback from employees and implement improvements                                           |
| **Determining Compensation**   | Set adequate salary and wage systems                                                                       |
| **Making Work Interesting**    | Make workplace environment fun and interesting                                                             |

**Key Insight:** Better the motivation schemes, better the quality of work produced.

---
## Types of Motivation
### Intrinsic vs. Extrinsic Motivation

| Aspect             | Intrinsic Motivation                              | Extrinsic Motivation                          |
| ------------------ | ------------------------------------------------- | --------------------------------------------- |
| **Source**         | Internal (from within)                            | External (from outside)                       |
| **Driver**         | Interest, enjoyment, personal satisfaction        | Rewards, incentives, fear of punishment       |
| **Control**        | Self-controlled                                   | Controlled by external source                 |
| **Example**        | "I want to complete this task because I enjoy it" | "Complete this task and you will be rewarded" |
| **Sustainability** | Long-lasting                                      | Temporary                                     |

**Intrinsic Motivation:** Individual's motivational thoughts come from within; driven by interest or enjoyment in the task itself; doesn't rely on external rewards; engages willingly to improve skills.

**Extrinsic Motivation:** Desires to perform tasks are controlled by an external source; someone else wants you to do something, offering certain rewards or fear of punishment.

---
### Positive vs. Negative Motivation

| Aspect             | Positive Motivation               | Negative Motivation                   |
| ------------------ | --------------------------------- | ------------------------------------- |
| **Approach**       | Rewards, incentives, recognition  | Fear, punishment, threats             |
| **Mindset**        | "I want to complete this task"    | "Complete this task or you are fired" |
| **Effect**         | Creates enthusiasm and commitment | Creates compliance through fear       |
| **Sustainability** | Long-term                         | Short-term                            |

---
## Types of Motivation by Focus
### Attitude Motivation
**Definition:** About how people think and feel; self-confidence, belief in themselves, attitude to life (positive or negative).

**Key Points:**
- How people feel about their future
- How they react to their past
- If both employers and employees maintain **positive attitude** about themselves, others, and situations → leads to better performance
---
### Group Motivation
**Definition:** Teams are the life of any organization; productivity and success depend upon employee performance as a team.

**Key Points:**
- Team leaders must understand how to influence team members' motivation
- Bringing out the best in each individual and recognizing their performance increases motivation
- Team building exercises, trainings, incentives, social recognition help motivate members
---
### Executive Motivation
**Definition:** One of the key issues in organizations is motivating and keeping managerial talents.

**Motivation Techniques for Executives:**
- Profit sharing incentives
- Bigger and challenging responsibilities
- Share options
- Benefits (medical, vehicle)
- Public recognition
- Better pay
---
## Motivation Theories
### Maslow's Hierarchy of Needs Theory
**Theorist:** Abraham Maslow (1940s)

**Core Idea:** Employee behavior is motivated by several needs arranged in a hierarchy. Lower needs must be at least partially satisfied before higher needs become motivating.

**The Hierarchy:**

| Level | Need Type               | Description                                                                       | Organizational Application                                          |
| ----- | ----------------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| **5** | **Self-Actualization**  | Feeling that full potential has been realized; only a few people reach this level | Challenging and meaningful work requiring innovation and creativity |
| **4** | **Esteem Needs**        | Personal achievement, recognition, respect, self-esteem                           | Recognize achievements publicly; assign important work              |
| **3** | **Social Needs**        | Belonging to a group, acceptance, friendship, love                                | Teamwork, social events, group activities                           |
| **2** | **Safety Needs**        | Protection from physical danger, economic security                                | Safe working conditions, pensions, protection plans                 |
| **1** | **Physiological Needs** | Basic needs for survival (food, water, shelter, sleep)                            | Comfortable workspace, adequate wages, suitable working hours       |

**Key Point:** This is called the **"satisfaction progression process"** because once a need is satisfied, it no longer motivates, and the person progresses to the next level.

---
### Alderfer's ERG Theory
**Theorist:** Clayton Alderfer (1969)

**Core Idea:** Human needs are divided into three categories.

| Category            | Description                                                   | Corresponding Maslow Needs             |
| ------------------- | ------------------------------------------------------------- | -------------------------------------- |
| **E - Existence**   | Physical needs (food, shelter, clothing, sleep, safety)       | Physiological + Safety needs           |
| **R - Relatedness** | Interpersonal needs within personal and professional settings | Social + Esteem needs (external)       |
| **G - Growth**      | Personal development and self-fulfillment                     | Esteem (internal) + Self-Actualization |

**Key Point:** Unlike Maslow, ERG theory allows movement between levels and recognizes that if higher needs are frustrated, individuals may regress to lower needs.

---
### McClelland's Theory of Learned Needs
**Theorist:** David McClelland (1960s)

**Core Idea:** Three needs are important sources of motivation that are learned through life experiences.

| Need                            | Characteristics                                                                                                                 | Work Preferences                                                |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| **Need for Achievement (nAch)** | Desire to set and accomplish challenging goals; takes calculated risks; likes regular feedback; assumes personal responsibility | Prefers challenging tasks; wants feedback; takes responsibility |
| **Need for Power (nPow)**       | Desire to control environment, people, and resources; some seek power for societal good; some for personal benefit              | Enjoys competing and winning; seeks leadership positions        |
| **Need for Affiliation (nAff)** | Wants to belong to a group; forms better work relationships; wants to be liked and respected                                    | Avoids conflict; seeks harmony; prefers collaborative work      |

---
### McGregor's Theory X and Theory Y
**Theorist:** Douglas McGregor (1960s)

**Core Idea:** Managers' assumptions about human nature influence their management style.

**Theory X Assumptions (Negative View):**

| Assumption                                                   | Implication                         |
| ------------------------------------------------------------ | ----------------------------------- |
| Average person dislikes work and attempts to avoid it        | Need to push and persuade employees |
| Has no ambition, wants no responsibility                     | Prefers to follow rather than lead  |
| Is self-centered and doesn't care about organizational goals | Needs external motivation           |
| Resists change                                               | Needs direction and control         |
| Avoids making decisions; not particularly intelligent        | Needs close supervision             |

**Management Style Required:** Strict control, supervision, direction, threats of punishment, coercion.

**Theory Y Assumptions (Positive View):**

| Assumption                                                    | Implication                        |
| ------------------------------------------------------------- | ---------------------------------- |
| Doesn't necessarily hate work; work can be as natural as play | Can find satisfaction in work      |
| Will be self-directed to meet work objectives if committed    | Needs commitment, not control      |
| Desires to achieve organizational goals                       | Internal motivation                |
| Will seek responsibility                                      | Can handle autonomy                |
| Makes decisions with respect to commitment                    | Can participate in decision making |

**Management Style Required:** Less control; gives employees more control and responsibility; creates environment for greater job satisfaction and high motivation.

---
### Fear and Punishment Theory (Reinforcement Theory)
**Theorist:** B.F. Skinner

**Core Idea:**
- Just as Theory X assumes, some individuals dislike work, hate responsibility, and avoid challenging jobs
- For these people, fear and punishment schemes are necessary to motivate them
- Some people get motivated by rewards; some must be controlled through fear
---
### Herzberg's Two-Factor (Motivation-Hygiene) Theory
**Theorist:** Frederick Herzberg (1959)

**Core Idea:** Workplace has two separate sets of factors:

1. **Hygiene Factors** (Dissatisfiers)
2. **Motivators** (Satisfiers)

| Factor Type         | Characteristics                                                                          | Examples                                                                         | Effect on Job Satisfaction                         |
| ------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------- |
| **Hygiene Factors** | Lower-level needs; when adequate → no dissatisfaction; when inadequate → dissatisfaction | Job security, working conditions, company policies, relationships, salary, wages | Prevents **dissatisfaction** but does NOT motivate |
| **Motivators**      | Higher-level needs; when present → satisfaction; when absent → no satisfaction           | Achievement, recognition, responsibility, growth opportunities, advancement      | Creates **satisfaction** and motivates             |

**Key Insights:**
- **Hygiene factors** prevent dissatisfaction but don't motivate
- **Motivators** create satisfaction and genuine motivation
- **Salary** is primarily a hygiene factor (prevents dissatisfaction)
- **Recognition and achievement** are motivators
---
### Vroom's Expectancy (VIE) Theory
**Theorist:** Victor Vroom (Yale School of Management)

**Core Idea:** A person will decide to behave or act in a certain way based on what they expect the result of that behavior will be.

**Formula:** **Motivation = Expectancy × Instrumentality × Valence**

| Component               | Description                                                       | Formula Element            | Example                                   |
| ----------------------- | ----------------------------------------------------------------- | -------------------------- | ----------------------------------------- |
| **Expectancy (E)**      | Person's perception that effort will result in performance        | E = (Effort → Performance) | "If I try hard, I can achieve the target" |
| **Instrumentality (I)** | Person's perception that performance will lead to desired outcome | I = (Performance → Reward) | "If I achieve target, I'll get bonus"     |
| **Valence (V)**         | Value the individual places on the reward                         | V = Value of Reward        | "I really want that bonus"                |

**Important:** If any factor is zero, motivation becomes zero (M = 0).

**Management Implications:**
- **Expectancy:** Discover what resources, training, or supervision employees need
- **Instrumentality:** Ensure promised rewards are fulfilled and employees are aware
- **Valence:** Understand what employees value based on their needs, goals, and values

**Why It's Called a Process Theory:** Explains the cognitive process through which needs are translated into behavior.

---
## Motivation Techniques

| Technique                                    | Description                                                         |
| -------------------------------------------- | ------------------------------------------------------------------- |
| **Meaningful and Challenging Work**          | When employees feel their work has impact, they become more engaged |
| **Clear Targets and Expectations**           | Employees must know what is expected; measure performance           |
| **Regular, Direct, and Supportive Feedback** | Timely, specific feedback about behavior or skills                  |
| **Valuing Employees**                        | Give credit, respect, and recognize contributions                   |
| **Training and Development Programs**        | Prepare employees for future; shows investment in them              |
| **Incentive Programs**                       | Rewards, promotions, benefits, profit sharing, freedom              |
| **Employee Participation**                   | Involve employees in decision making                                |
| **Recognition and Awards**                   | Formal acknowledgment of achievement                                |

---
# LEADERSHIP
## Introduction to Leadership
**Definition:**
- The process of influencing people and providing an environment for them to achieve team or organizational activities
- The activity of leading a group of people or an organization
- An art of motivating a group of people to act towards achieving a common goal

**Key Points:**
- Leader is an inspiration and director of action
- Leaders arrange the work environment (allocating resources, defining organizational goals)
- Top management should have sharp leadership skills
- Without better leadership, the company will struggle to sustain in the long run
- Great leaders possess the ability to adapt their behavior and styles according to the situation

**Leader vs. Manager:**

| Aspect              | Leader                          | Manager                             |
| ------------------- | ------------------------------- | ----------------------------------- |
| **Role**            | Influences and inspires         | Holds position of authority         |
| **Focus**           | People and vision               | Tasks and results                   |
| **Source of Power** | Personal influence and charisma | Position and formal authority       |
| **Approach**        | Inspirational                   | Administrative                      |
| **Key Skill**       | Emotional intelligence          | Technical and administrative skills |

**Note:** All leaders are not necessarily managers; a leader might be any team member with strong influence over others.

---
## Qualities of a Good Leader

| Quality                      | Description                                                                             |
| ---------------------------- | --------------------------------------------------------------------------------------- |
| **Honesty and Integrity**    | Trustworthy; same on outside and inside; ethical behavior; lives life with honesty      |
| **Self-Confidence**          | Confident in person and leadership roles; conveys confidence towards proposed objective |
| **Human Skills**             | Knows how to treat and talk to members; gives respect and credit; treats people fairly  |
| **Initiative**               | Takes initiative; constructs and implements own plans; does right thing at right time   |
| **Innovative**               | Good imagination and visualization; combines new ideas with old; develops new tactics   |
| **Communication Skills**     | Effective and excellent communicator; good speaker and writer; uses simple language     |
| **Discipline**               | Disciplined person; respects rules and regulations; followers will follow example       |
| **Commitment to Excellence** | Maintains high standards; proactive in raising the bar; works alongside team members    |
| **Vision**                   | Clear sense of direction and purpose                                                    |
| **Decisiveness**             | Makes timely decisions                                                                  |
| **Resilience**               | Overcomes setbacks                                                                      |
| **Empathy**                  | Understands and relates to others                                                       |

---
## Leadership Styles
### Autocratic (Authoritarian) Style
**Characteristics:**
- Gives orders which must be obeyed
- Makes choices based on own ideas and judgments
- Rarely accepts advice from followers
- Little or no input from group members

**When Beneficial:**
- Decisions need to be made quickly
- When consulting large groups is impractical
- Emergency situations
- Inexperienced employees

**Drawbacks:**
- Can be viewed as bossy, controlling, dictatorial
- Leads to resentment among group members
- Discourages creativity
- Low employee morale

**Difference Between Boss and Leader:**

| Boss (Autocratic)    | Leader               |
| -------------------- | -------------------- |
| Gives orders         | Inspires action      |
| Demands respect      | Earns respect        |
| Focuses on authority | Focuses on influence |
| Tells what to do     | Shows how to do      |

---
### Democratic (Participative) Style
**Characteristics:**
- Gives orders after consulting the group
- Policies worked out in group discussions
- Praise and blame shared by group members
- Members encouraged to share ideas and opinions
- Leader retains final say over decisions

**Benefits:**
- One of the most effective leadership styles
- Higher productivity
- Better contributions from group members
- Increased group morale
- Members feel more engaged
---
### Laissez-Faire (Free Rein) Style
**Characteristics:**
- Leader provides little or no direction
- Gives employees maximum freedom
- Depends on group to establish own goals and solve problems
- Group members provide their own motivation

**When Appropriate:**
- Highly skilled professionals
- Creative work
- Self-motivated teams

**Not Appropriate When:**
- Group members lack knowledge or experience
- Decisions need to be made quickly
- Team lacks self-motivation
---
### Blake and Mouton's Managerial Grid
**Theorists:** Robert R. Blake and Jake Mouton
**Core Idea:** Leadership style is determined by two dimensions:
- **X-axis:** Concern for Production (1 to 9)
- **Y-axis:** Concern for People (1 to 9)

**The Grid:**
![concern for production](attachmnets/concern%20for%20production.png)

|Style|Grid Position|Description|Characteristics|
|---|---|---|---|
|**Impoverished**|1,1|Low concern for both production and people|Main concern: not be held responsible for mistakes; less innovation; minimal effort|
|**Country Club**|1,9|Low concern for production; high concern for people|Pays attention to employee comfort; friendly atmosphere; not necessarily productive|
|**Task/Perish**|9,1|High concern for production; low concern for people|Employee needs unimportant; provides money and expects performance; uses rules and punishments|
|**Middle-of-the-Road**|5,5|Moderate concern for both|Tries to balance company goals and workers' needs; achieves adequate performance|
|**Team/Sound**|9,9|High concern for both production and people|Encourages teamwork and commitment; most effective; maximum performance through committed people|

**Significance for Organization's Growth:**
- **9,9 Team Management** is considered most effective
- Promotes both productivity and employee satisfaction
- Creates synergy where people work together effectively
- Develops committed and motivated workforce
- Leads to sustainable organizational growth
---
## Leadership Approaches/Theories
### Trait Approach
**Core Idea:** Leadership is based on personal traits, abilities, personality, background, physical characteristics, intelligence, attitudes, learning behavior, and maturity.

**Core Traits of Effective Leaders:**

| Trait                              | Description                                                                   |
| ---------------------------------- | ----------------------------------------------------------------------------- |
| **Achievement Drive**              | High level of effort, ambitions, inner motivation to pursue goals             |
| **Leadership Motivation**          | Intense desire to lead others to fulfill organizational goals                 |
| **Honesty and Integrity**          | Trustworthy, reliable; truthfulness and tendency to translate word into deeds |
| **Self-Confidence**                | Belief in oneself, ideas, and ability                                         |
| **Intelligence/Cognitive Ability** | Capable of exercising good judgment; strong analytical abilities              |
| **Knowledge of Business**          | Knowledge of organization and other technical matters                         |
| **Self-Monitoring Personality**    | Sensitivity to situational cues and ability to adapt behavior appropriately   |

**Limitation:** Not all leaders share same traits; traits alone don't guarantee effectiveness.

---
### Behavioral Approach
**Core Idea:** Focuses on what leaders do (behavior); argues great leaders are made, not born; leadership capability can be learned.

**Two Distinguished Behaviors:**

| Behavior Type       | Description                                                                | Example Actions                                                                  |
| ------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| **People-Oriented** | Mutual trust and respect for subordinates; genuine concern for their needs | Listens to suggestions, does personal favors, treats equally, supports interests |
| **Task-Oriented**   | Focus on getting work done; ensures rules and procedures are followed      | Assigns specific tasks, pushes performance, emphasizes deadlines                 |

**Key Insight:** Initially thought leaders who are people-oriented are weak at task-oriented (and vice versa), but studies show effective leaders can exhibit **high levels of both**; these behaviors are independent.

---
### Contingency/Situational Approach
**Core Idea:** Best approach to leadership depends upon the situation. What a leader does depends on characteristics of the situation.

**Leadership Styles in Contingency Approach:**

| Style                    | Description                                                                                                | When Appropriate                   |
| ------------------------ | ---------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| **Directive**            | Task-oriented; explains what task is to be completed, how, and when; maintains clear performance standards | When employees need clear guidance |
| **Supportive**           | People-oriented; friendly and approachable; shows respect; treats employees equally                        | When work is stressful or boring   |
| **Participative**        | Consults with employees; listens to suggestions; considers ideas before making decisions                   | When employees are knowledgeable   |
| **Achievement-Oriented** | Sets challenging goals; expects high performance; shows confidence in workers                              | When tasks are complex             |

---
### Integrated Approach to Leadership
**Core Idea:** Integration of:
- Characteristics, behavior, and experience of the **Leader**
- Ability, experience, and maturity of **Followers**
- Structure, technology, objectives, and external **Situation**

**Diagram:**
**Key Insight:** Most realistic and effective approach to leadership.

---

# ENTREPRENEURSHIP
### C.1 Introduction to Entrepreneurship
**Etymology:** French word _'entrependre'_ which means "to undertake."

**Definition:**  
The capacity and willingness to develop, organize, and manage a business venture along with its risks in order to make a profit.

**Key Concepts:**
- In simple terms: starting a new business
- Ranges from solo projects to major undertakings creating many job opportunities
- An entrepreneur is a person of high aptitude who pioneers change
- Someone who wants to work for themselves and start their own business with their knowledge and innovations
---
## Entrepreneurial Characteristics

| Characteristic            | Description                                                               |
| ------------------------- | ------------------------------------------------------------------------- |
| **Initiative**            | Takes action beyond job description; does things before asked             |
| **Opportunity Seeking**   | Quick to seize opportunities; acts before demanded by situation           |
| **Discipline/Confidence** | Takes steps daily toward objectives; confident in making business succeed |
| **Risk Taking**           | Calculates risks before taking action; moderate risk-taker (not gambler)  |
| **Creative/Innovative**   | Lacks imagination? Then not an entrepreneur; finds solutions to problems  |
| **Competitive**           | Knows they can do a better job; has desire to win and be successful       |
| **Committed/Determined**  | Committed to excellence; tries again and again until success              |
| **Strong People Skills**  | Knows how to motivate employees; highlights benefits; coaches others      |
| **Strong Work Ethics**    | First to arrive, last to leave; mind constantly on work; motivates team   |

---
## Entrepreneurship Development
**Role in Economic Development:**
- Vital role in economic development of a country
- Involves adoption of new business organizations, new technologies, new enterprises
- Creates goods not previously available
- Identifies and seizes opportunities for economic benefit

**Factors Influencing Entrepreneurship:**

| Factor                        | Description                                                             |
| ----------------------------- | ----------------------------------------------------------------------- |
| **Creativity and Innovation** | Sources of new ideas; transforming ideas into opportunities             |
| **Inner Drive**               | Working for oneself; using knowledge to benefit oneself and the economy |
| **Education and Skills**      | Knowledge and training that enable opportunity recognition              |
| **Access to Resources**       | Capital, infrastructure, and support systems                            |
| **Supportive Environment**    | Government policies, market conditions, and cultural acceptance         |

---
## Need for Promotion of Entrepreneurship

| Reason                         | Description                                                                                                                        |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- |
| **Economic Growth**            | Entrepreneurship supports economic growth; governments should develop entrepreneurial thinking                                     |
| **Employment Generation**      | Creates jobs; saves corporate sector from troubled economic times (unemployment, downsizing)                                       |
| **Small Business Development** | Small businesses are essential for future economic prosperity                                                                      |
| **Innovation and Change**      | According to Joseph Schumpeter: introduces new production methods, develops new markets, finds fresh raw materials, brings changes |
| **Youth Empowerment**          | Global Entrepreneurship Week exposes youth to benefits of entrepreneurship; motivates creativity and innovation                    |

**Joseph Schumpeter's View:** Functions of entrepreneurship include:
- Introduction of new methods of production
- Developing new markets
- Finding fresh sources of raw materials
- Making and bringing changes

---
## Importance of Entrepreneurship in Nepali Context
**For National Economy:**
- Employment creation for young workforce
- Economic diversification beyond agriculture
- Innovation and technology development
- Export potential for software and services
- Regional development
- Poverty reduction through income generation
- Self-sufficiency and reduced imports

**For IT Sector in Nepal:**
- Creates innovative software and services
- Generates employment opportunities
- Attracts investment and talent
- Develops technical skills in workforce
- Contributes to economic growth
- Reduces brain drain
- Promotes digital transformation
---
## Steps for Establishing a Small Scale Unit
**Who Can Start?**
- Existing entrepreneur or fresh to business
- With or without business background in family
- Educated or uneducated
- Rural/backward area persons, physically handicapped, minorities
- **Key Requirements:** Strong will, essential skills, ability to work hard, ability to take calculated risks
#### Step-by-Step Process:

| Step                          | Description                                                                                                                                                             |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1. Product Identification** | Design a new product or imitate established product; conduct market research; understand competition                                                                    |
| **2. Forms of Ownership**     | Decide on sole proprietorship, partnership, or corporation; should be economically viable                                                                               |
| **3. Location**               | Choose considering: proximity to raw materials, nearness to market, transportation, labor availability, power/energy supplies, disposal facilities, political patronage |
| **4. Approvals**              | Government approvals, taxation, rules and regulations, licensing, registration, paperwork                                                                               |
| **5. Finance**                | Arrange fixed capital (land, building, machinery) and working capital (operating costs, raw materials); long-term loans from private parties or financial institutions  |
| **6. Production Management**  | Allocate space for operations; choose production method; follow quality testing practices; buy raw materials; hire required help                                        |
| **7. Marketing**              | Decide prices for products/services; ensure flow of revenues; plan for profit margin                                                                                    |

**Key Advice:** Planning in advance is useful. Keep assessing and improving your plan at every stage.

---
## Risks and Challenges for Entrepreneurs
**For Aspiring Entrepreneurs:**
- Financial constraints (student loans, lack of capital)
- Lack of experience and skills
- Social stigma around failure
- Preference for secure employment
- Family expectations for stable employment
- Limited mentorship and networks
- Access to finance
- Regulatory compliance burden
- Infrastructure challenges (unreliable electricity, internet)

**For IT Sector Entrepreneurs in Nepal:**
- Infrastructure challenges
- Limited venture capital
- Talent retention (brain drain)
- Small domestic market
- Technology adoption challenges
- Bureaucracy
- Payment systems limitations
- Skills gap
- Political instability
---
