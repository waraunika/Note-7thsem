...
## ⁠C.2. Importance/Benefits of S Parameters
...
1. It is useful in frequency domain analysis and not time domain analysis
2. Most silicon loads are non-linear. Moreover, most digital circuits operate using voltage thresholds. These applications require time domain analysis which is not possible using S-parameters
3. Conversion between time domain and frequency domain introduces errors.
4. Though it is possible to cascade S-parameters, it gets messy. Hence it is done in a better way using ABCD parameters.
5. It is not possible to describe a network in both voltage and current waves using S-parameters. For this, ABCD parameters are used.

## ⁠C.4. Relation between \[S\] and \[Z\]

- We have seen that \[Z\] relates to the total voltages and currents at the various ports,
...
| Feature |Hybrid (H) Parameters | Scattering (S) Parameters |
| --- | --- | --- |
|Variable Type| Total voltages (V) and currents (I) | Incident and reflected power/voltage waves (a and b) | 
| Termination State| Open-circuit (I=0) and short-circuit (V=0) | Matched characteristic impedance (Z₀, e.g., $50\Omega$ ) |
| Operating Frequency | Low-frequency, audio, and DC (e.g., BJTs) | High-frequency, RF, microwave, and millimeter-wave |
| Measurability | Hard at high frequencies due to parasitics from OC/SC | Easy to measure using a Vector Network Analyzer (VNA) |
| Units in Matrix | Mixed units (ohms, siemens, and dimensionless ratios) | Dimensionless complex voltage/power ratios |

## ⁠C.5. Average Power in terms of scattering variables
...
## ⁠D.1. Insertion Loss (or Transmission Loss)

- Supposing $P_{in 1}$ is the power fed to port 1 from a matched source
- $P_{out 2}$ is the power output measured by a power meter (matched to output port), then
    $$\begin{align}\dfrac{P_{out 2}}{P_{in 1}} = \dfrac{|b_2|^2}{|a_1|^2} = |S_{21}|^2 \end{align}$$
- The insertion loss (aka transmission loss), expressed in dB can be obtained as:
    $$\begin{align} S_{21}\ \text{(dB)} = - 20 \log_{10} |S_{21}| = -10 \log_{10}\left( \dfrac{P_{out 2}}{P_{in 1}}\right) \end{align}$$

## ⁠D.2. Return Loss (or Refelection Loss)
...
- The magnitude of the reflection coefficient at port 1, denoted as $|S_{11}|$ can be obtained from,
    $$\begin{align}\dfrac{P_{r1}}{P_{in1}} = \dfrac{|b_1|^2}{|a_1|^2} = |S_{11}|^2 \end{align}$$
- The return loss (or reflection loss) of the device expressed in dB is given by
    $$\begin{align} S_{11}\ \text{(dB)} = - 20 \log_{10} |S_{11}| = -10 \log_{10}\left( \dfrac{P_{r1}}{P_{in1}}\right) \end{align}$$
- The input VSWR of a device when the output port is matched terminated, is given by:
    $$\begin{align} \text{VSWR} = \dfrac{1 + |S_{11}|}{1 - |S_{11}|} \end{align}$$
...
Given the s-parameters

$$[S] = \begin{bmatrix} 0.4 + j 0.5 & j 0.5 \\ j 0.5 & 0.4 - j 0.5 \end{bmatrix}$$

Find, if the matrix is: (a) Reciprocal, (b) Lossless (c) Find the return loss at input (d) Find reflected power if input power is 5 W.
...
$$\begin{align}
[S][S]*
&= \begin{bmatrix} 0.4 + j0.5 & j0.5 \\ j0.5 & 0.4 - j0.5 \end{bmatrix} 
\times\ \begin{bmatrix} 0.4 - j0.5 & j0.5 \\ j0.5 & 0.4 + j0.5 \end{bmatrix}\\
&= \begin{bmatrix} 0.16 + 0.25 - 0.16 & j0.2 - 0.25 + j0.2 - 0.25\\ 
j0.2 + 0.25 + j0.2 + 0.25 & -0.25 + 0.16 + 0.25 \end{bmatrix}\\
&= \begin{bmatrix} 0.25 & -0.5 + j0.4 \\ 0.5 + j0.4 & 0.16 \end{bmatrix}
\end{align}
$$

Since $[S][S]*$ is not equal to $[I]$, the junction isn't lossless.
